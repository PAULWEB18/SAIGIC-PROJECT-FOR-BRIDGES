...
Public Class FrmTransfertCacao
...
#End Region
    ...
    Private Sub spvchargerInterfaces()
        ...
        listeSites.LoadSites()
        spbChargerDropDownList(listeSites, cboSiteDepart, "IDSite", "NomCourt")
        cboSiteDepart.SelectedIndex = -1

        'ListeMagasins.LoadMagasins()
        'spbChargerDropDownList(ListeMagasins, cboSiteDepart, "NomMagasin", "NomMagasin")
        'cboSiteDepart.SelectedIndex = -1
    ...
    Private Sub FrmTransfert_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        Try
    ...
    Private Sub FrmTransfert_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub FrmTransfertCacao_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Return Then
            SendKeys.Send("{TAB}")
        End If
    End Sub

    Private Sub FrmTransfert_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub spvChargerPesee(ByVal bvlCode As String)
        ...
        With MaPesee
            ...
            cboVehicule.SelectedValue = .Immatriculation
            cboRemorque.SelectedValue = .Remorque
            ...
            txtLieuDepart.Text = .ProvenanceDepartement
            cboSiteDepart.SelectedIndex = cboSiteDepart.FindString(.ProvenanceLocalite)
            ...
            cboTransporteur.SelectedIndex = cboTransporteur.FindString(.Transporteur)
            ...
            Else
                txtNatureProduit.Text = ""
            End If
            txtCodePeseeOrigine.Text = .CodePeseeLien
    ...
    Protected Function AjouterPesee() As Boolean
        ...
        Try
            tmpReponse = LesPesees.AddBObjectAndBDataPesee(TxtNumeroPesee.Text, _
                                          ...
                                          txtCodePeseeOrigine.Text, _
                                          "TRANSFERT", _
                                          mskAnneeRecolte.Text, _
                                          txtLieuDepart.Text, _
                                          cboSiteDepart.Text, _
    ...
    Protected Function ModifierPesee() As Boolean
        ...
        Try
            ...
            LesPesees.UpdateBObjectProvenanceDepartement(TxtNumeroPesee.Text, txtLieuArrivee.Text)

            LesPesees.UpdateBObjectFournisseur(TxtNumeroPesee.Text, cboCodeFournisseur.SelectedValue)
            LesPesees.UpdateBObjectNumeroTicket(TxtNumeroPesee.Text, txtNumeroTicket.Text)
            LesPesees.UpdateBObjectCodePeseeLien(TxtNumeroPesee.Text, txtCodePeseeOrigine.Text)
            ...
            LesPesees.UpdateBObjectProvenanceLocalite(TxtNumeroPesee.Text, cboSiteDepart.Text)
    ...
    Private Function ValideSaisiesPesees() As Boolean
        ...
        If Me.cboSiteDepart.SelectedValue = Nothing Then
            Me.ErrorProvider1.SetError(Me.cboSiteDepart, "Champs obligatoire.")
            Me.cboSiteDepart.Focus()
            ValideSaisiesPesees = False
        End If
        ...
        If Me.txtLieuDepart.Text = "" Then
            Me.ErrorProvider1.SetError(Me.txtLieuDepart, "Champs obligatoire.")
            Me.txtLieuDepart.Focus()
            ValideSaisiesPesees = False
        End If

        If Me.txtCodePeseeOrigine.Text = "" Then
            Me.ErrorProvider1.SetError(Me.txtCodePeseeOrigine, "Champs obligatoire.")
            Me.txtCodePeseeOrigine.Focus()
            ValideSaisiesPesees = False
        End If
    ...
    Private Sub cboProduit_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboProduit.SelectedIndexChanged
        If cboProduit.Text = "CACAO BROUSSE" Then
            txtNumeroLot.Text = "BROUSSE"
            'txtNumeroLot.ReadOnly = True
        Else
            txtNumeroLot.Text = ""
            'txtNumeroLot.ReadOnly = False
            'Chargement des lots usines
        End If
    End Sub


    Private Sub TextBox1_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtCodePeseeOrigine.KeyPress
        Dim Permis = "qwertyuioplkjhgfdsazxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM0123654789-" & Chr(13) & Chr(8)
        If InStr(Permis, e.KeyChar) = 0 Then
            e.KeyChar = ""
        End If
    End Sub

    Private Sub cboSiteDepart_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboSiteDepart.SelectedIndexChanged

        Dim tmpZP As String = ""

        Dim tmpLesSite As New BObject.Magasin
        Dim SelSite As New BObject.Magasin
        'tmpLesSite.LoadSites()
        Try
            'tmpZP = tmpLesSite.FindSite(cboSiteDepart.SelectedValue).ZonePortuaire
            tmpZP = ListeMagasins.FindMagasin(cboSiteDepart.SelectedValue).Localisation
        Catch ex As Exception

        End Try
        txtLieuDepart.Text = tmpZP

        If tmpZP <> "" Then
            txtLieuDepart.ReadOnly = True
        Else
            txtLieuDepart.ReadOnly = False
        End If
    End Sub
